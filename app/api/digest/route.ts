...
export async function GET(req: NextRequest) {
  ...
  for (const pref of prefs ?? []) {
    ...
    (meals ?? []).forEach((m:any) => { byType[m.meal_type] = (byType[m.meal_type] ?? 0) + Number(m.servings_available ?? 0); });
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#173c2b">
        ...
        <p style="font-size:24px"><strong>${coverage} días</strong> de cobertura estimada</p>
